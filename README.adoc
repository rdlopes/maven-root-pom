...
* **Build Standards**: Java 25+, Maven 3.9.10+, UTF-8 encoding
...
* **Maven**: 3.9.10+ (enforced by Maven Enforcer Plugin)
...
* **JUnit** (BOM imported)
* **AssertJ** (BOM imported)
* **Cucumber** (BOM imported)
...
* **JaCoCo**: Configured to capture coverage for both unit tests (`jacoco-ut.exec`) and integration tests (`jacoco-it.exec`).
* **Aggregation**: Automatically merges all test data into a unified `jacoco.exec` file during the `post-integration-test` phase, generating a combined HTML report in the `verify` phase under `target/site/jacoco/`.
...
**Environment**:
* Ubuntu Latest * Java 25 (Temurin distribution) * Maven cache enabled
...
* Use `./mvnw` wrapper for a consistent Maven version
